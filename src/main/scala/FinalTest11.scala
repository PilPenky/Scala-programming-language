// Структура JSON немного отличается. Удалены лишние пробелы после значений и перед двоеточиями. Нужно реализовать отступ после красной строки в 4 пробела (вместо 2)
import Color._
import FinalTest11.system.log
...
object XmlUtils {
  ...
  def formatJsonString(jsonString: String): String = {
    // Парсим JSON обратно в AST, чтобы переформатировать с 4 пробелами
    parse(jsonString) match {
      case Right(json) =>
        // Принтер с 4 пробелами вместо 2
        val printer = Printer.spaces2.copy(indent = "    ")
        val jsonWith4Spaces = printer.print(json)

        // Убираем лишние пробелы у ключей и двоеточий
        val fixedColons = jsonWith4Spaces.replaceAll("([\"\\w])\\s+:", "$1:")
        val fixedKeys = fixedColons.replaceAll("\"(data|id|validate|idValid|isValid|values|restId)\"\\s+:", "\"$1\":")

        fixedKeys

      case Left(error) =>
        log.error(s"Ошибка парсинга JSON для форматирования: ${error.getMessage}")
        jsonString // Возвращаем как есть в случае ошибки
    }
  }
  ...
  override def receive: Receive = {
    case ConvertXmlToJson(xmlPath, jsonPath) =>
      try {
        ...
        val jsonString = json.spaces2 // Стандартное форматирование с отступами
        val formattedJson = XmlUtils.formatJsonString(jsonString) // Убираем лишние пробелы
...
object FinalTest11 extends App {
