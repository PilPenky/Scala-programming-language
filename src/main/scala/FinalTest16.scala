/*  Готово. Произвести "покраску" кода */
import Color._
import FinalTest16.system.log
...
object DataComparator {
  ...
  /**
   * Сравнивает XML-файл с JSON-ответом сервера.
   *
   * @return true, если данные идентичны
   */
  def compare(xmlPath: String, serverJson: String): Boolean = {
    val xmlData = parseXml(xmlPath)
    val jsonData = parseJson(serverJson)
    xmlData == jsonData
  }
...
class MainActor extends Actor with ActorLogging {
  ...
  private def compareData(): Unit = {
    ...
    log.info(s"${Color.colorResultStart}Результат сравнения: ${if (isEqual) "ДАННЫЕ ИДЕНТИЧНЫ!" else "ДАННЫЕ РАЗЛИЧАЮТСЯ!"}${Color.colorResultEnd}")
...
object FinalTest16 extends App {
