/*  Структура JSON идентична с серверной. Реализовать отправку JSON-файла на сервер.
    Отправка данных произошла, но порядок данных не сохранился. */
import Color._
import FinalTest13.system.log
...
class HttpClientActor extends Actor with ActorLogging {
  ...
  private def postDataToServer(json: Json): Future[String] = {
    ...
    val entity = HttpEntity(ContentTypes.`application/json`, json.noSpaces)

    log.debug(s"Sending JSON: ${json.noSpaces.take(500)}...") // Логируем часть отправляемых данных
    ...
    Http().singleRequest(HttpRequest(
      ...
      case HttpResponse(status, _, entity, _) =>
        Unmarshal(entity).to[String].flatMap { body =>
          log.error(s"Server responded with $status: $body")
          Future.failed(new RuntimeException(s"Server error: $status - $body"))
        }
    }
  }
    ...
    case ConversionSuccessful(jsonPath) =>
      ...
      parse(jsonString) match {
        case Right(json) =>
          ...
          val dataArray = json.hcursor.downField("data").as[List[Rest]].getOrElse(Nil)

          if (dataArray.nonEmpty) {
            // Отправляем каждый Rest объект отдельно
            dataArray.foreach { rest =>
              log.info(s"Sending rest with id: ${rest.restId}")
              httpClient ! PostData(rest.asJson)
            }
          } else {
            log.error("No valid data found in JSON")
...
object FinalTest13 extends App {
